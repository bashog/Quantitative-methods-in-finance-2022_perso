...
#####Problem 3#####
group <- c("man","man","man","man","man","man","woman","woman","woman","woman","woman","woman","woman","woman")
ResultsVector <- NULL
for(k in 1:50000){
  selection <- NULL
  for(i in 1:5){
...
sum(ResultsVector) / length(ResultsVector)
...
for(k in 1:1000){
  stock <- 100
  for(i in 1:100){
    stock <- stock + rnorm(1, mean = 0, sd = 7)
  }
  ResultsVector2 <- c(ResultsVector2, 120 - stock)
}

sum(ResultsVector2) / 100
